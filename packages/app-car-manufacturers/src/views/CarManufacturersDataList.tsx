...
const CarManufacturersDataList: React.FunctionComponent<Props> = ({ sortBy, setSortBy, limit, sorters }) => {
            ...
            showConfirmation(async () => {
                await deleteCarManufacturer({
                    ...
                    update: (cache, response) => {
                        ...
                        showSnackbar(t`CarManufacturer "{title}" deleted.`({ title: carManufacturer.title }));
                ...
                <ScrollList data-testid="carManufacturer-data-list">
                    {(data || []).map(item => (
                        <ListItem key={item.id} selected={item.id === id}>
                            <ListItemText onClick={() => history.push(`/carManufacturers?id=${item.id}`)}>
                                {item.title}
                                {item.description && (
                                    <ListItemTextSecondary>
                                        {item.description}
