...
export interface IDdbPutItemConditionalCheckFailedErrorParams {
    error: Pick<Error, "message">;
}

export const ddbPutItemConditionalCheckFailed = ({
    error
}: IDdbPutItemConditionalCheckFailedErrorParams) => {
    const { message } = error;
